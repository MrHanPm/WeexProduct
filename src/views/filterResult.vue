<template>
    <div class="filter-result">
        ...
        <list @loadmore="loadmoreData" @scroll="scroll" loadmoreoffset="88" offset-accuracy="40" ref="list">
            <header ref="header">
                <!--页面标题-->
            ...
            <cell class="result">
                ...
                <div class="result-list">
                    ...
                    <div :class="['series-item', index ? '' : 'item-first' ]" v-for="(result, index) in info.productList" v-if="!resultHasSeries" @click="goModelPage(result.id)">
                        <image class="result-pictrue" placeholder="https://s.kcimg.cn/wap/images/detail/productApp/placeholder.png" :src="result.src" resize="cover"></image>
                        <div class="result-cont">
...
<script type="text/babel">
    ...
    export default {
        ...
        methods:{
            ...
            // 请求初次加载页面的数据（包括点击用途类型时候的页面重载）
            requestInitApi () {
                // 根据获取筛选结果页面数据
                this.getData(`${this.ajaxUrl(false)}/index.php?r=weex/list&subCateId=${this.subCateId}&brandId=${this.brandId}&paramId=${this.getParamId()}`, res => {
                    if (res.ok) {
                        // 定义映射dta中的参数列表、品牌名、参数名、车系列表、子类id
                        let { cateInfo, paramList, brandName, paramName, seriesList, subCateId } = res.data
                        // 设置大类id 供大数据统计使用
                        ...
                        // 判断是否是品牌筛选
                        if (this.brandId) {
                            paramList.splice(1, 0, {'name': brandName, 'selected': true})
                        } else {
                            paramList.splice(1, 0, {'name': '品牌'})
                        }
                        // 如果是价格筛选要设置价格
            ...
            computeTypeHeight (paramList = []) {
                ...
                this.heightMax = 20 + Math.ceil( length / 3 ) * 84
                // 显示全部时候高度等于最大高度
                if (isShowAll) {
            ...
            showSidebar (fid, index) {
                ...
                if (this.hasPurpose && !index) {
                    // 请求用途类别数据
                    this.getData(`${this.ajaxUrl(false)}/index.php?r=weex/list/cate&brandId=${this.brandId}&paramId=${this.getParamId()}`, res => {
                        if (res.ok) {
                            let list = res.data
                            // 添加不限选项
                            list.unshift({
                ...
                // 有用途类别时候品牌index是2 反之是1
                let brandIndex = this.hasPurpose ? 2 : 1

                if (index === brandIndex) {
                    // 请求品牌子类数据
                    this.getData(`${this.ajaxUrl(false)}/index.php?r=weex/list/brand-filter&subCateId=${this.subCateId}&paramId=${this.getParamId()}`, res => {
                        if (res.ok) {
                            // 默认品牌是不限的
                    ...
                    // 请求其他子类数据
                    this.getData(`${this.ajaxUrl(false)}/index.php?r=weex/list/param-filter&subCateId=${this.subCateId}&fid=${fid}&paramId=${this.getParamId()}&brandId=${this.brandId}`, res => {
                        if (res.ok) {
                            let { params } = res.data
                            // 添加不限选项
                            params.list.unshift({
            ...
            updateData (isLoadmore = false) {
                ...
                const url = `${this.ajaxUrl(false)}/index.php?r=weex/list&isList=1&subCateId=${this.subCateId}&paramId=${this.getParamId()}&brandId=${this.brandId}&order=${this.order}&page=${this.currentPage}`
                this.getData(url, res => {
                    if (res.ok) {
                        this.lastUrl = url.replace(/&page=\d+$/,'')
            ...
            getSeriesSiderbarData (subCateId, seriesId) {
                this.getData(`${this.ajaxUrl(false)}/index.php?r=weex/list/product&subId=${subCateId}&seriesId=${seriesId}&paramId=${this.getParamId()}&brandId=${this.brandId}`, res => {
                    if (res.ok) {
            ...
            sendBigData (seriesSiderbarData) {
                ...
                if (seriesSiderbarData) {
                    param.p4 = seriesSiderbarData.filter(x => x).join('_')
                    param.p3 = 2
                } else {
                    let [ big, sub, brand ] = ['', '', '']
                    ...
                    param.p4 = [big, sub, brand].filter(x => x).join('_')
                    param.f = this.currentPage
                }
                console.log('发送大数据统计', param)
                this.collect(param)
            },
        },
        created () {
